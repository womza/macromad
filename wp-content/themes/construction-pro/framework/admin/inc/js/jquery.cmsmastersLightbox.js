	...
	var CmsmastersLightbox = function (element, parameters) { 
		...
		obj.methods = { 
			...
			openLightbox : function (data) { 
				...
				obj.methods.body.find('#cmsmastersLightBox_' + obj.methods.uniqID).find('.cmsmastersLightBoxContInMid').append(obj.methods.fields);
			...
			generateField : function (val) { 
				...
				fieldContent += '<div class="icons_list_parent">' + 
					...
					'<div class="cmsmasters_icon_search">' + 
						'<label>' + cmsmasters_admin_lightbox.find_icons + ':</label>' + 
		...
		privateMethods = { 
			...
			attachGeneratedEvents : function () { 
				...
				// Icon Choose
				$('#cmsmastersLightBox_' + obj.methods.uniqID + ' .icons_list_parent > ul > li > a').on('click', function () { 
